...
__all__ = ['plot_lines','plot_lines2', 'plot_bar_chart', 'plot_cdf']
...
def plot_lines2(resultset, desc, filename, plotdir):
    """Plot a graph with characteristics described in the plot descriptor out
    of the data contained in the resultset and save the plot in given directory.
    ...
    if 'yticks' in desc:
        ...
        ax1.set_yticklabels([str(ytick) for ytick in desc['yticks']])
    ymetrics = desc['ymetrics']
    zmetrics = desc['zmetrics']
    wmetrics = desc['wmetrics']
    #print(ymetrics , zmetrics)
    #ymetrices = desc['ymetrics']
    #ymetrices[1] = ('LATENCY', 'MEAN')
    #ymetrices[0] = ('LINK_LOAD', 'MEAN_INTERNAL')
    ycondnames = desc['ycondnames'] if 'ycondnames' in desc else None
    ycondvals = desc['ycondvals'] if 'ycondvals' in desc else None
    if ycondnames is not None and ycondvals is not None:
        if not len(ymetrics) == len(ycondnames) == len(ycondvals):
    ...
    for i in range(len(yvals)):
        ...
        for j in range(len(xvals)):
            ...
            ymetrices[0] = ('LINK_LOAD', 'MEAN_INTERNAL')
            ymetrices[1] = ('LATENCY', 'MEAN')
            data1 = [v.getval(ymetrices[0])
                    for _, v in resultset.filter(condition)
                    if v.getval(ymetrices[0]) is not None]
            data2 = [v.getval(ymetrices[1])
                    for _, v in resultset.filter(condition)
                    if v.getval(ymetrices[1]) is not None]   

            '''     
            confidence = desc['confidence'] if 'confidence' in desc else 0.95

            data1 = [v.getval(ymetrics[i])
                    for _, v in resultset.filter(condition)
                    if v.getval(ymetrics[i]) is not None]

            data2 = [v.getval(zmetrics[i])
                    for _, v in resultset.filter(condition)
                    if v.getval(zmetrics[i]) is not None]

            data3 = [v.getval(wmetrics[i])
                    for _, v in resultset.filter(condition)
                    if v.getval(wmetrics[i]) is not None]

            #norm = [float(i)/sum(raw) for i in raw]
            #data1norm = [float(i)/sum(data1) for i in data1]
            #data2norm = [float(i)/sum(data2) for i in data2]

            #normalized1 = [(float(i)-min(data1))/(max(data1)-min(data1)) for i in data1]
            #normalized2 = [(float(i)-min(data2))/(max(data2)-min(data2))  for i in data2]

            #data = normalized1 + normalized2
            #data3i = 1./data3
            #data = data1 + data2 + 1/data3
            norm1 = data1 / np.linalg.norm(data1)
            norm2 = data2 / np.linalg.norm(data2)
            norm3 = np.linalg.norm(data3) / data3 
            norm = norm1 + norm2+norm3

            means[j], err[j] = means_confidence_interval(norm, confidence)
        ...
        if all(np.isnan(x) for x in xvals) or all(np.isnan(y) for y in means):
            ...
            #plt.errorbar(xvals, means, yerr=yerr, fmt=fmt, **plot_args)
            plt.errorbar(xvals, means, fmt=fmt, **plot_args)
            empty = False
    if empty and not plot_empty:
        return
    x_min = desc['xmin'] if 'xmin' in desc else min(xvals)
